...
def sample_gamma(alpha, beta):
    """Sample from gamma distribution. Copied from RecurrentGPT."""
    if alpha < 1:
...
def sample_negative_binomial(t, target_mean):
    """Sample from negative binomial distribution. Copied from RecurrentGPT."""
...
class ModernBertForMaskedLMWithRecycling(ModernBertPreTrainedModel):
    ...
    def __init__(self, config: ModernBertConfig):
        ...
        self.config = config
        self.model = ModernBertModelWithRecycling(
            config)
...
class ModernBertModelWithRecycling(ModernBertPreTrainedModel):
    ...
    def __init__(self, config: ModernBertConfig):
        ...
        # Split layers into prelude, core_block, and coda
        n_prelude = config.n_layers_in_prelude
        n_recurrent = config.n_layers_in_recurrent_block
        n_coda = config.n_layers_in_coda
        
        prelude = nn.ModuleList()
        for layer_id in range(n_prelude):
            prelude.append(ModernBertEncoderLayer(config, layer_id))
        ...
        if config.injection_type == "linear":
            ...
            adapter = nn.Sequential(
                nn.Linear(config.hidden_size * 2, config.hidden_size, bias=config.attention_bias),
                nn.GELU(),
        ...
        for layer_id in range(n_recurrent):
            core_block.append(ModernBertEncoderLayer(config, layer_id + n_prelude))
        
        coda = nn.ModuleList()
        offset = n_prelude + n_recurrent
        for layer_id in range(n_coda):
            coda.append(ModernBertEncoderLayer(config, layer_id + offset))
    ...
    def forward(
        ...
        # Prelude: process through prelude layers
        for encoder_layer in self.transformer.prelude:
    ...
    def randomized_iteration_sampler(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not lockstep_k and torch.distributed.is_initialized():
        ...
        if "curriculum-" in self.config.sampling_scheme:
            ...
            if self.step > ramp_length:
                t = max(self.config.mean_recurrence - self.config.backprop_depth, 0)
                s = self.config.backprop_depth
            else:
                slope = self.step / ramp_length
                t = max(math.ceil(slope * (self.config.mean_recurrence - self.config.backprop_depth)), 0)
                s = max(math.ceil(slope * self.config.backprop_depth), 1)
        else:
            t = max(self.config.mean_recurrence - self.config.backprop_depth, 0)
            s = self.config.backprop_depth

        if self.training:
            if "uniform" in self.config.sampling_scheme:  # truly uniform: n is uniform, k is fixed from config
                # Parse custom ranges from scheme, e.g. "uniform-1-4" (only for n, k is always fixed)
                scheme = self.config.sampling_scheme
                
                # k is always fixed from config (backprop_depth), not sampled
                k = torch.as_tensor(s)
                
                # Check for explicit range specification for n
                if scheme.startswith("uniform-") and "n:" not in scheme:
                    # Format: "uniform-1-4" (only for n, k is fixed from config)
                    # Remove "uniform-" prefix and split by "-"
                    range_part = scheme.replace("uniform-", "")
                    parts = range_part.split("-")
                    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                        n_min, n_max = int(parts[0]), int(parts[1])
                        n = torch.randint(low=n_min, high=n_max + 1, size=(1,), generator=n_generator)
                    else:
                        # Fallback to default range for n
                        n = torch.randint(low=0, high=max(2 * t, 1), size=(1,), generator=n_generator)
                else:
                    # Default: use t based range for n, k is fixed from config
                    n = torch.randint(low=0, high=max(2 * t, 1), size=(1,), generator=n_generator)
            elif "bptt" in self.config.sampling_scheme:  # skewed toward n+k ~ max_recurrence
                ...
                # Parse max_total_steps from scheme if provided, e.g. "poisson-lognormal-filling-12"
                # The sampled value represents total steps (n + k), not just n
                scheme = self.config.sampling_scheme
                max_total_steps = 12  # Default upper bound for total steps (n + k)
                if "-" in scheme and scheme.split("-")[-1].isdigit():
                    max_total_steps = int(scheme.split("-")[-1])

                sigma = 0.5
                # Use s (backprop_depth) as mean for total steps, so mean is 4
                mu = math.log(max(s, 1)) - (sigma**2 / 2)
                rate = torch.zeros((1,)).log_normal_(
                    mean=mu, std=sigma, generator=n_generator)
                total_steps = torch.poisson(torch.tensor(
                    [rate], dtype=torch.float), generator=n_generator)
                # Bound total_steps between 1 (min 1 grad step) and max_total_steps
                total_steps = torch.clamp(total_steps, min=1, max=max_total_steps)
                
                # k (grad steps) is min(total_steps, backprop_depth), max is backprop_depth (e.g. 4)
                k = torch.minimum(total_steps, torch.as_tensor(s))
                # n (no grad steps) is the remainder
                n = torch.clamp(total_steps - k, min=0)
    ...
    def initialize_state(self, input_embeds):
        """Initialize recurrent state. Copied from RecurrentGPT.
        
        Initializes the recurrent state tensor based on the state_init config.
        Different initialization strategies can help with training stability and convergence.
        """
        ...
        elif state_init == "embed":  # initialized like a scaled embedding:
            x = torch.randn_like(input_embeds).mul(1 / math.sqrt(input_embeds.shape[-1]))
        elif state_init == "like-init":
            x = torch.randn_like(input_embeds)
            # Use the same std as embeddings (from initializer_range)
            std = self.config.initializer_range
            cutoff_factor = getattr(self.config, 'initializer_cutoff_factor', 3)
            torch.nn.init.trunc_normal_(x, mean=0.0, std=std, a=-cutoff_factor * std, b=cutoff_factor * std)
            if self.emb_scale != 1:
                x = x * self.emb_scale
        elif state_init == "zero":
            x = torch.zeros_like(input_embeds)
        elif state_init == "unit":
            x = torch.randn_like(input_embeds)
            std, mean = torch.std_mean(x, dim=-1, keepdim=True)
            x = (x - mean) / std
        else:
            # Default fallback: use input_embeds as starting point
    ...
    def iterate_forward(self, input_embeds, position_embeddings, attention_mask, sliding_window_mask, cu_seqlens, max_seqlen, position_ids: Optional[torch.LongTensor] = None, output_attentions: Optional[bool] = None, num_steps_pair: Optional[torch.Tensor] = None):
        ...
        if getattr(self.config, 'randomize_embed_step', False):
